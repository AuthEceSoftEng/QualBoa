...
public class Scorer {
	...
	public void quickSortHighToLow(String[] content,float[] arr, int low, int high) {
	...
	public void quickSortLowToHigh(String[] content,float[] arr, int low, int high) {
		if (arr == null || arr.length == 0)
			return;
	...
	public void postProcessor(String[] fileContents,float[] score){
		...
		for (int i=0;i<fileContents.length;i++){
			...
			if (counter>1){
				quickSortLowToHigh(fileContents,loc, i, i+counter-1);
				i+=counter-1;
			}
		}
